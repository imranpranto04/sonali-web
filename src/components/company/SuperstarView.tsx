...
import { SuperstarData, Achiever } from "@/lib/api/company-service";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { User, Building2, Trophy, MapPin } from "lucide-react";
...
export default function SuperstarView({ data }: { data: SuperstarData }) {
  ...
  return (
    <div className="w-full">
      ...
      >
        {/* --- FLOATING TABS --- */}
        <div className="bg-white/80 backdrop-blur-xl p-1.5 rounded-full shadow-xl shadow-brand/5 mb-12 inline-flex border border-white/50 sticky top-24 z-30">
          <TabsList className="bg-transparent h-auto p-0 gap-1">
            {data.categories.map((cat) => (
              ...
              >
                {cat.label === "Employee of the Month"
                  ? "Employees"
                  : cat.label === "Top Agents"
                  ? "Agents"
                  : cat.label}
              </TabsTrigger>
            ))}
        ...
        {data.categories.map((cat) => (
          ...
          >
            {cat.achievers.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {cat.achievers.map((person) => (
                  <CompactEliteCard key={person.id} achiever={person} />
                ))}
              </div>
            ) : (
...
// --- COMPACT ELITE CARD ---
function CompactEliteCard({ achiever }: { achiever: Achiever }) {
  return (
    <div className="group relative bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-brand/10 transition-all duration-300 hover:-translate-y-1 flex flex-col mt-5">
      {/* 1. IMAGE SECTION (Square Ratio - shorter) */}
      <div className="relative aspect-square w-full bg-slate-100 overflow-hidden">
        {/* Top linear Line */}
        <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-amber-400 to-amber-600 z-30" />
        ...
        {/* Hover Overlay */}
        <div className="absolute inset-0 bg-linear-to-t from-brand/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" />

        {/* Hover Reveal Badge */}
        <div className="absolute bottom-3 left-3 z-20 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 delay-75">
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white text-[9px] font-bold uppercase tracking-wider">
            <Trophy className="w-3 h-3 text-amber-400" /> Star
          </span>
        </div>
      </div>

      {/* 2. DETAILS SECTION (Compact) */}
      <div className="p-5 flex flex-col grow relative bg-white">
        {/* Header */}
        <div className="text-center mb-4">
          <h3 className="text-base font-bold text-brand leading-snug mb-1 line-clamp-1 group-hover:text-amber-600 transition-colors">
            {achiever.name}
          </h3>
        ...
        {/* Footer Info (Side by Side to save height) */}
        <div className="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
          {/* Left: Branch/Dept */}
          {(achiever.branch || achiever.DeptName) && (
            <div className="flex items-center gap-1.5 text-xs text-slate-500 overflow-hidden">
              <MapPin className="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <span
                className="truncate max-w-[100px] text-[11px] font-medium"
                title={achiever.branch || achiever.DeptName}
              >
                {achiever.branch || achiever.DeptName}
              </span>
            </div>
          ...
          {/* Right: Score/Premium */}
          {achiever.performance && (
            <div className="flex items-center gap-1.5 text-xs text-brand shrink-0">
